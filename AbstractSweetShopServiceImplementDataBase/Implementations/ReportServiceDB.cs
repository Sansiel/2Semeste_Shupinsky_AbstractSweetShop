...
namespace AbstractSweetShopServiceImplementDataBase.Implementations
{
    public class ReportServiceDB : IReportService
    {
        ...
        public void SaveCandyPrice(ReportBindingModel model)
        {
            ...
            try
            {
                ...
                var Candies = context.Candies.ToList();
                //создаем таблицу
                var paragraphTable = document.Paragraphs.Add(Type.Missing);
                var rangeTable = paragraphTable.Range;
                var table = document.Tables.Add(rangeTable, Candies.Count, 2, ref
                missing, ref missing);
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = Candies[i].CandyName;
                    table.Cell(i + 1, 2).Range.Text = Candies[i].Price.ToString();
                }
        ...
        public List<StoreLoadViewModel> GetStoreLoad()
        {
            ...
            new StoreLoadViewModel
            {
                ...
                Materials = StoreCompList.Select(r => new Tuple<string,
    int>(r.Material.MaterialName, r.Count))
        ...
        public List<BuyerJobsViewModel> GetBuyerJobs(ReportBindingModel model)
        {
            return context.Jobs
            .Include(rec => rec.Buyer)
            .Include(rec => rec.Candy)
            .Where(rec => rec.DateCreate >= model.DateFrom &&
            rec.DateCreate <= model.DateTo)
            .Select(rec => new BuyerJobsViewModel
            {
        ...
        public void SaveBuyerJobs(ReportBindingModel model)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].CandyName, fontForCells));
                table.AddCell(cell);
